    ...
    <div className={`bg-white shadow-xs`}>
      <div className="flex sm:flex-wrap lg:flex-no-wrap">
        ...
        <div className="flex flex-col p-6 sm:text-center lg:text-left pt-20">
          <h5 className="text-2xl font-extrabold uppercase" >{"We take conduct at the Modern web summit very seriously. please take the time to read it!"}</h5>
          {CodeOfConductList.map((p,i) =>
            <React.Fragment key={i}>
              {p.point && <p className={'text-gray-300 font-extrabold text-base lg:w-3/4 sm:w-full'}>{p.point}</p>}
              {p.dot && <p className={'text-gray-300 text-base lg:w-3/4 sm:w-full'}>•{p.dot}</p>}
              {p.text && <p className={'text-gray-300 font-medium text-base lg:w-3/4 sm:w-full'}>{p.text}</p>}
              <br />
            </React.Fragment>
          )}
        </div>
